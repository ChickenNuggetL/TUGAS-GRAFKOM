...
public class LinkKirby {
    ...
    public void walkAnim(boolean isStopped) {
        ...
        if (!isStopped) {
            ...
            if (currentAnimTime <= animDuration / 4f) {
                // 1st Move pos
                //kirby leftfoot
                KirbyRootBody.get(0).getChildObject().get(0).rotationObject((float) Math.toRadians(leftFootFrameTime), 0f, -1f, 0f);
                //kirby rightfoot
                KirbyRootBody.get(0).getChildObject().get(1).rotationObject((float) Math.toRadians(rightFootFrameTime), 1f, 0f, 0f);
                ...
                // Rev Move pos
                //kirby leftfoot
                KirbyRootBody.get(0).getChildObject().get(0).rotationObject((float) Math.toRadians(rightFootFrameTime), 1f, 0f, 0f);
                //kirby rightfoot
                KirbyRootBody.get(0).getChildObject().get(1).rotationObject((float) Math.toRadians(leftFootFrameTime), 0f, -1f, 0f);
    ...
    public void EAT(boolean isStopped) {
        float animDuration2 = 14f;
        Object Mouth = KirbyRootBody.get(0).getChildObject().get(6);
        if (!isStopped) {
            float inc = 30f / 1000f;
            if (eatcurrentAnimTime < animDuration2 / 2f) {
                //Mouth.translateObject(-15 / 1000f, 0 / 1000f, 0 / 1000f);
                Mouth.scaleObject2(1000f / 1000f + inc, 1000f / 1000f + inc, 1000f / 1000f + inc);
            } else if (eatcurrentAnimTime < animDuration2 && eatcurrentAnimTime >= animDuration2 / 2f) {
                Mouth.scaleObject2(1000f / 1000f - inc, 1000f / 1000f - inc, 1000f / 1000f - inc);
                //Mouth.translateObject(0 / 1000f, 0 / 1000f, 0 / 1000f);
            } else if (eatcurrentAnimTime >= animDuration2) {
                eatcurrentAnimTime = 0f;
            }
            eatcurrentAnimTime += 1f;
        }
    }
